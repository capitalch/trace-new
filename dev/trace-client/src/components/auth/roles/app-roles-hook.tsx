...
import { AppGridSearchBox, useComponentHistory, useAgGridUtils, useFeedback, useAppGraphql, useCellRenderers, appStore,useDialogs, Messages, GraphQlQueryResultType, AppGridToolbar, } from '@src/features'
import {_, ColDef, GridOptions, GridReadyEvent,  useGranularEffect, useRef, Button,  AgGridReact, Flex,  HStack, useState} from '@src/libs'
...
function useAppRoles() {
    ...
    useGranularEffect(() => {
        appStore.superAdmin.roles.doReload = loadData
    }, [], [loadData])
    ...
    async function loadData() {
        const args = {
            sqlId: 'get_roles',
            sqlArgs: {
                clientId: appStore.login.clientId || 0
            }
        }
        ...
        try {
            ...
            if (rows) {
                ...
                appStore.superAdmin.roles.doFilter()
            }
        } catch (e: any) {
    ...
    async function onSubmit() {
        ...
        try {
            ...
            handleUpdateResult(result, () => {
                closeModalDialogA()
                appStore.superAdmin.roles.doReload()
            }, 'genericUpdate')
        } catch (e: any) {
