import { AppGridToolbar, AppStoreType, useCellRenderers, useComponentHistory, useAgGridUtils, useFeedback, useAppGraphql, appStore, appStaticStore, useDialogs, Messages, GraphQlQueryResultType } from '@src/features'
import { AgGridReact, EditIcon, useGranularEffect, useRef, Flex, IconButton, Tooltip, ColDef, GridOptions, GridReadyEvent, RowDataUpdatedEvent, State, useHookstate } from '@src/libs'
...
function SuperAdminClients() {
    ...
    const { DeleteCellRenderer, HideCellRenderer }
        = useCellRenderers({
            dbName: 'traceAuth'
            , tableName: 'ClientM'
            , appStoreObject: store.superAdmin.clients
    ...
    return (
        <Flex h='100%' direction='column' className="ag-theme-balham" >
            <AppGridToolbar appStoreObject={store.superAdmin.clients} appStaticStoreObject={appStaticStore.superAdmin.clients} title='All clients view' CustomControl={SuperAdminNewClientButtons} gridApiRef={gridApiRef} />
            <AgGridReact
                ...
                ref={gridApiRef}
                // rowData={store.superAdmin.clients.filteredRows.value}
    ...
    async function loadData() {
        const args = {
            ...
            sqlArgs: {
                noOfRows: store.superAdmin.clients.noOfRows.value || null
            }
        }
        ...
        try {
            ...
            if (rows) {
                store.superAdmin.clients.rows.set(rows)
